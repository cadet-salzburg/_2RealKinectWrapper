...
---------------
upcoming
	+ test near mode and implement it for new microsoft pc kinect hardware
	+ get highres modes implemented and tested for prime sense device (1600x1200)
	+ init and restart single devices
	+ centerOfMass calculation for MS kinect manually via moment calculation of user image
	+ record and playback (probably just openni)
	+ open for suggestions

0.9.4 Update Release
	+ use boost::shared_ptr and boost::shared_arrays
	+ mutexes changed to scoped mutexes for thread and exception safety
	+ fixed getNrOfUsers (before it called the same code as getNrOfSkeletons)
	+ added feature getUsersWorldCenterOfMass (just openni support for now)
	+ some cosmetics
	+ improved debug output of multiple kinect sample (shows center of masses, nr of users and nr of skeletons)
